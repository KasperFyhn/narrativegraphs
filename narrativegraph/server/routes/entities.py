...
def get_document_ids_by_entity(entity_id: int, limit: Optional[int],
                               db_service: DbService = Depends(get_db_service)) -> list[int]:
...
def get_entity_labels_by_ids(entity_ids: list[int], db_service: DbService = Depends(get_db_service)) -> list[
    EntityLabel]:
...
async def get_entity(entity_id: int, db_service: DbService = Depends(get_db_service)):
...
async def get_docs_by_entity(
    ...
    doc_ids = get_document_ids_by_entity(entity_id, limit, db_service)

    if len(doc_ids) == 0:
        raise HTTPException(status_code=404, detail="No documents found.")

    # Call your existing getDocs function
    docs = get_docs_by_ids(db_service.session, doc_ids, None)
    return docs


@router.post("/labels", response_model=list[EntityLabel])
async def get_entity_labels(
        request: EntityLabelsRequest, db_service: DbService = Depends(get_db_service)
):
    """Get entity labels by IDs"""
    db = db_service.session

    entity_labels = get_entity_labels_by_ids(request.ids, db_service)
